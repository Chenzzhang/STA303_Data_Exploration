...
- \usepackage{fancyhdr}
- \pagestyle{fancy}
- \fancyhead[CO, CE]{Chen Zhang, 1004784646}
...
reported <- reported_raw %>%
  ...
  mutate(type = fct_relevel(type, 'Active', 'Recovered', after=0))
```
...
reported %>% 
  ggplot()+
  geom_bar(aes(x = reported_date, y = count, fill = type), stat = "identity")+
  labs(title = 'Cases reported by day in Toronto, Canada',
       subtitle = 'Confirmed and probable cases',
       x = 'Date',
       y = 'Case count',
       caption = str_c('Created by: Chen Zhang for STA303/1002, U of T\n', 
...
outbreak %>% 
  ggplot()+
  geom_bar(aes(x = episode_week, y = cases, fill = outbreak_or_sporadic), 
           stat= "identity")+
...
income <- nbhood_profile %>%
  filter(Topic == 'Low income in 2015', Characteristic == '  18 to 64 years (%)') %>%
  filter(`_id` == 1143) %>%
  pivot_longer(-c(`_id`, Topic, `Data Source`, Characteristic, Category), 
                names_to = 'neighbourhood_name', 
                values_to = 'Low_Income_Count') %>%
  select(-1:-5) %>%
  mutate(Low_Income_Count = parse_number(Low_Income_Count))
```
...
nbhoods_all <- nbhoods_shape_raw %>%
  ...
  mutate(neighbourhood_name = str_replace(neighbourhood_name, "St.James Town", 
                                          "St. James Town"))%>%
...
```{r nbhood_dw_3}
nbhood_final <- nbhoods_all %>%
  mutate(med_inc = median(Low_Income_Count), med_rate = median(rate_per_100000)) %>%
  group_by(AREA_NAME) %>%
  mutate(nbhood_type = case_when(
        (Low_Income_Count >= med_inc& rate_per_100000 >= med_rate) ~ 
...
nbhood_final %>%
  ggplot() +
  geom_sf(aes(fill =  Low_Income_Count)) +
  scale_fill_gradient(name = '% low income', 
                      low = 'darkgreen', 
                      high = 'lightgrey') +
  labs(title = 'Percentage of 18 to 64 years olds living in a low income family (2015)',
...
nbhood_final %>%
  ...
  labs(title = 'COVID-19 cases per 100,000, by neighbourhood in Toronto, Canada',
       caption = str_c('Created by: Chen Zhang for STA303/1002, U of T\n', 
                       'Income data source: Census Profile,',
                       ' 98-316-X2016001 via OpenData Toronto\n', 
